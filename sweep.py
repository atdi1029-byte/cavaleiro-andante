...
def build_query_trails(lat, lng, radius):
    r = radius
    return (
        f"[out:json][timeout:45];"
        f"("
        ...
        f'way["highway"="footway"]["name"]["foot"!="no"](around:{r},{lat},{lng});'
        f');\nout center 200;'
    )
...
def save_places(places):
    # Read existing file and replace only the SWEPT_PLACES section
    # so WIKI_PLACES, PARK_PLACES, SUNSET_PLACES etc. are preserved
    if os.path.exists(PLACES_JS):
        with open(PLACES_JS) as f:
            content = f.read()
        new_block = "const SWEPT_PLACES = " + json.dumps(places, indent=2, ensure_ascii=False) + ";\n"
        if re.search(r'const SWEPT_PLACES\s*=\s*\[', content):
            content = re.sub(r'const SWEPT_PLACES\s*=\s*\[.*?\];\n', new_block, content, flags=re.DOTALL)
        else:
            content = new_block + content
    else:
        content = "// Auto-generated — do not edit by hand\n\n"
        content += "const SWEPT_PLACES = " + json.dumps(places, indent=2, ensure_ascii=False) + ";\n"
...
def sweep_zone(zone, existing_places, existing_names):
    ...
    for label, qfn in [("nature", build_query_nature), ("historic", build_query_historic), ("trails", build_query_trails)]:
        try:
